...
robot_id = Robot_IDClass3.Robot_unique().read_robot_id()
...
# 3.24.25 robot heartbeat integration
...
async def write_sensor_data(data):
    ...
    try:
        ...
        payload = {
            "mac": robot_id,  # Using robot_id as MAC
            **data  # Spread all the sensor data
        }
        
        # Make POST request to update bot data
...
async def sensor_data_loop():
    ...
    while True:
        try:
            ...
            # Only send data if we have some sensor readings
            if sensor_data:
